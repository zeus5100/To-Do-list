...
<div class="container mx-auto max-w-md mt-10 p-6 bg-white rounded shadow">
    <h1 class="text-2xl font-semibold mb-6 text-center">Edytuj zadanie</h1>
    ...
    <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" maxlength="255" required class="w-full px-3 py-2 border rounded @error('name') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('name', $task->name) }}">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" class="w-full px-3 py-2 border rounded @error('description') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $task->description) }}</textarea>
        ...
        <div>
            ...
            <select name="priority" id="priority" required class="w-full px-3 py-2 border rounded @error('priority') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $priority->value }}" @selected(old('priority', $task->priority) == $priority->value)>
        ...
        <div>
            ...
            <select name="status" id="status" required class="w-full px-3 py-2 border rounded @error('status') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $status->value }}" @selected(old('status', $task->status) == $status->value)>
        ...
        <div>
            ...
            <input type="date" name="completion_date" id="completion_date" required class="w-full px-3 py-2 border rounded @error('completion_date') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('completion_date', $task->completion_date) }}">
